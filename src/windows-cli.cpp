...
void CallShotup(const std::wstring args, bool wait)
{
    // generate full path for shotup executable
...
// Console 'wrapper' for shotup on windows
int wmain(int argc, wchar_t* argv[])
{
    // if no args, do not wait for stdout
    if (argc == 1) {
        std::cout << "Starting shotup in daemon mode" << std::endl;
        CallShotup(L"", false);
    } else {
        std::wstring argString = joinArgs(argc, argv);
        CallShotup(argString, true);
    }
    std::cout.flush();
    return 0;
}
