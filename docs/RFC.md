...
[Shotup RFCs]: #shotup-rfcs

Many changes, including bug fixes and documentation improvements can be
implemented and reviewed via the normal GitHub pull request workflow.

Some changes though are "substantial", and we ask that these be put through a
bit of a design process and produce a consensus among the Shotup community and development team.
...
Shotup or the RFC process itself. What constitutes a
...
Some changes do not require an RFC:
  ...
  + Additions only likely to be _noticed by_ other developers-of-shotup,

    invisible to users-of-shotup.
...
the idea over on our [official Matrix Space](https://matrix.to/#/#shotup-org:matrix.org) in the [Developers](https://matrix.to/#/!AYbNXDGTDwApLwzkNg:feneas.org?via=kde.org&via=matrix.org&via=ryan77627.xyz), or opening an issue on [Github Discussions](https://github.com/shotup-org/shotup/discussions).

## What the process is

[What the process is]: #what-the-process-is

In short, to get a major feature added to Shotup, one must first get the RFC
merged into the RFC repository as a markdown file. At that point the RFC is
"active" and may be implemented with the goal of eventual inclusion into Shotup.

  + Fork the Shotup repo
...
feature as a pull request to the Shotup repo. Being "active" is not a rubber
...
associated issue tracking its implementation in the Shotup repository; thus that
associated issue can be assigned a priority via the triage process that the
team uses for all issues in the Shotup repository.
